...
class MainTests(unittest.TestCase):
    ...
    def test_book_detail_logged_in(self):
        """Test that the book appears on its detail page."""
        # Uses helper functions to create books, authors, user, & to log in
        create_books()
        create_user()
        login(self.app, 'me1', 'password')
    ...
    def test_create_author(self):
        ...
        post_data = {
            'name': 'Some Author',
            'biography': 'Some long bio...',
            'books': []
        }
        self.app.post('/create_author', data=post_data)

        # Verifys that the author was updated in the database
        created_author = Author.query.filter_by(name='Some Author').one()
        self.assertIsNotNone(created_author)
        self.assertEqual(created_author.biography, 'Some long bio...')
    ...
    def test_create_genre(self):
        ...
        # Verifys that the genre was updated in the database
    ...
    def test_profile_page(self):
        # Making a GET request to the /profile/me1 route
        create_books()
        create_user()
    ...
    def test_favorite_book(self):
        ...
        # Verifys that the book with id 1 was added to the user's favorites
        response = self.app.get('/profile/me1', follow_redirects=True)
        self.assertEqual(response.status_code, 200)
    ...
    def test_unfavorite_book(self):
        # Logs in as the user me1, and add book with id 1 to me1's favorites
        create_books()
        create_user()
        login(self.app, 'me1', 'password')
